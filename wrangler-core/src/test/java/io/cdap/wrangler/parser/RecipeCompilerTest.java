...
public class RecipeCompilerTest {
  ...
  public void testByteSizeAndTimeDurationParsing() throws Exception {
    // Test directives using the new BYTE_SIZE and TIME_DURATION tokens.
    String[] recipeValid = new String[] {
        "// Set column using a TIME_DURATION",
        "set-column :target_time 1.5hr;",
        "// Set column using a BYTE_SIZE",
        "set-column :target_size 10MB;",
        "// Use the new aggregate directive",
        "aggregate-stats :size :time total_size total_time;",
        "// Use a hypothetical directive taking these types directly",
        "some-directive 5kb 250ms :some_col;",
        "// Use another value type alongside",
        "set-column :another_val 'some string';"
    };
    // Ensure these valid recipes compile successfully
    TestingRig.compileSuccess(recipeValid);

    // Removing the previous compileFailure checks as they were based on
    // incorrect assumptions about identifier vs. specific type parsing at compile
    // time.
    // The grammar allows identifiers like 'KB' even if a specific directive might
    // reject it later.
  }
}
