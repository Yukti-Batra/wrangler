...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  /**
   * Visits the node representing a byte size argument (e.g., "10KB").
   * Creates a {@link ByteSize} token.
   */
  @Override
  public RecipeSymbol.Builder visitByte_size(DirectivesParser.Byte_sizeContext ctx) {
    builder.addToken(new ByteSize(ctx.getText(), TokenType.BYTE_SIZE));
    return builder; // Continue visiting children if any (though likely none for a terminal rule)
  }

  /**
   * Visits the node representing a time duration argument (e.g., "150ms").
   * Creates a {@link TimeDuration} token.
   */
  @Override
  public RecipeSymbol.Builder visitTime_duration(DirectivesParser.Time_durationContext ctx) {
    builder.addToken(new TimeDuration(ctx.getText(), TokenType.TIME_DURATION));
    return builder; // Continue visiting children if any
  }
