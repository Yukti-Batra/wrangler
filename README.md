...
## New Data Types: Byte Size and Time Duration (Added in vX.Y.Z)

Wrangler now supports parsing directive arguments representing byte sizes and time durations directly.

### Byte Size (`BYTE_SIZE`)

Arguments expecting a byte size can be specified using standard units. The value will be parsed and represented internally in bytes (using a 1024 base for K, M, G, etc.).
...
- **Supported Units:**
  ...
  - `t`, `T` (Terabytes)
  - `p`, `P` (Petabytes)
- **Optional Suffix:** `b` or `B` can be appended (e.g., `10KB` is the same as `10K`).
- **Examples:** `10KB`, `1.5MB`, `500k`, `2G`, `1.2TB`

### Time Duration (`TIME_DURATION`)

Arguments expecting a time duration can be specified using common time units. The value will be parsed and represented internally in nanoseconds.
...
- **Supported Units:**
  ...
  - `hr` (Hours)
  - `d`, `day`, `days` (Days)
- **Examples:** `150ms`, `2.5s`, `10sec`, `1.5min`, `2hr`, `3days`

## New Directive: `aggregate-stats`

This directive aggregates total byte sizes and time durations from specified columns across all processed rows.

**Syntax:**

```
aggregate-stats :<size-column> :<time-column> <target-size-column> <target-time-column>
```

- **`:<size-column>` (ColumnName):** The source column containing byte size strings (e.g., "10KB", "1.5MB"). Invalid or unparsable values in this column are treated as 0 bytes for the aggregation.
- **`:<time-column>` (ColumnName):** The source column containing time duration strings (e.g., "150ms", "2.5s"). Invalid or unparsable values in this column are treated as 0 nanoseconds for the aggregation.
- **`<target-size-column>` (Identifier):** The name of the new column to store the total aggregated size **in bytes**.
- **`<target-time-column>` (Identifier):** The name of the new column to store the total aggregated time **in nanoseconds**.

**Functionality:**

The directive iterates through all input rows, parsing the values in the specified source columns. It maintains running totals for the total bytes and total nanoseconds encountered. After processing all rows, it produces a single output row containing the two target columns populated with these final totals.

**Example Usage:**

```wrangler
# Sample recipe using aggregate-stats
parse-as-csv :body ',';
drop :body;
# Assuming columns 'data_size' (e.g., "512KB") and 'latency' (e.g., "85ms") exist
aggregate-stats :data_size :latency total_bytes_processed total_latency_ns;
# The output will be a single row with columns total_bytes_processed and total_latency_ns
```
